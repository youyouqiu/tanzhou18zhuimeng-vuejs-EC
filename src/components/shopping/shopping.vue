...
<template>
    <div class="shopping">
        ...
        <div>
            <wv-flex :gutter="10" class="shopflex" style="margin-left: 2px; margin-right: 2px;">
                <wv-flex-item class="shopwidth" style="padding-left: 2px; padding-right: 2px; flex: 1 1 auto;" v-for="(item, index) in shopList" :key="index">
                    <router-link :to="{name:'Shop.Shopdetails', query: {id: (index + 1), title:'likeYou'}}" :title="item.des">
                        <div class="placeholder">
                            <div class="shopimgs">
                                <img width="100%" height="100%" :src="item.imgUrl" v-lazy="item.imgUrl">
                            </div>
                            <p class="shoptitle" v-text="item.des"></p>
                            <div class="clear shoptext">
                                <span class="fl shoptext1" v-text="item.symbol"></span>
                                <span class="fl shoptext2" v-text="item.price"></span>
                                <span class="fr shoptext3" v-text="item.alreadyPaid"></span>
                            </div>
                        </div>
                    </router-link>
                </wv-flex-item>
    ...
    export default {
        ...
        data () {
            return {
                title: "潭州网上商城",
                shopList: [
                    {des: "【今日上新】日本-船新脱下来的热气腾腾花99999999999999999999999内裤01", imgUrl: "http://static.oeofo.com/201610/27/131242571000812.png", symbol: "$", price: "888", alreadyPaid: "00001"},
        ...
        created (){
            ...
            // function remChange(){
            //     let html = document.getElementsByClassName('shopping')[0];
            ...
            this.$ajax.get(url)
                .then(res => {
                    console.log("shopping页：雅蠛蝶，上面的报错提示是因为ajax没有数据");
                })
        }
    }
</script>
